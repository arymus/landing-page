async function getRepos() {
    ...
    try {
        ...
        if (res.status != 200) throw new Error("HTTP Error: " + res.status); // If the response is not 200 (OK), throw an error
...
async function displayRepos() {
    ...
    for (let repo of repos) { // Iterate over each repository
        ...
        for (i in lang) langText.innerText += lang[i] + ", ";  // Add the language to the element's innerText (plus a comma)
